...
TAV_SYSTEM_PROMPT = (
    ...
    "4. ONE PORT TYPE AT A TIME, LENGTH BEFORE VOLUME. Pick ONE port type -- input OR output -- "
    "and get it exact across EVERY case before you move to the other; finishing one port type "
    "before starting the other keeps your changes from fighting each other. WITHIN a port, FIRST "
    "make the vector LENGTH match the target on every case (len_delta = 0): a wrong length means "
    "your tree emits the wrong number of cycles, so the per-cycle values can never line up until "
    "the length is right. ONLY THEN drive the per-cycle (volume) deltas to 0. The overall score "
    "(sum over both ports of all cases) is what ultimately matters and must reach 0.\n"
